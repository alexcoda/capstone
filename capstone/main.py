...
def run_text_EWC_model(save_name, source_name, target_name, args):
    ...
    results_df = train_text_ewc(train_loaders, test_loaders, vocab_size, args, consolidate=True)

    save_results('ewc', save_name, results_df)
...
def run_EWC_model(save_name, source_name, target_name, args):
    ...
    results_df = train_ewc(train_datasets, test_datasets, args, consolidate=True)
...
def run_LWF_model(save_name, source_name, target_name, args):
    ...
    if not args.text_mode:
        ...
        train_loaders, test_loaders, vocab = get_text_dataloader(names, args)
        vocab_size = len(vocab)
        train_source_loader = train_loaders[0]
        train_target_loader = train_loaders[1]
        test_source_loader = test_loaders[0]
        test_target_loader = test_loaders[1]


    # Train the model
    results_df = train_LWF(train_source_loader, train_target_loader,
                           test_target_loader, test_source_loader, args, vocab_size=vocab_size)
...
def main(args):
    ...
    run_LWF_model(save_name, 'twitter', 'imdb', args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--text_mode', type=int, default=0, metavar='L',
                        help='Run on text datasets or vision datasets (1 for text datasets)')
